...
class LessonAndSubscriptionTestCase(APITestCase):
    ...
    def setUp(self):
        """Подготовка данных и аутентификация пользователей."""
        ...
        self.course = Course.objects.create(title="Test Course", owner=self.moderator)
    ...
    def test_lesson_youtube_link_only(self):
        ...
        data = {"title": "Bad Link", "video_url": "https://example.com", "course": self.course.id}
    ...
    def test_lesson_create_valid_link(self):
        ...
        self.client.force_authenticate(user=self.user)
        data = {"title": "Good Lesson", "video_url": "https://youtube.com", "course": self.course.id}
        response = self.client.post("/api/lessons/", data)
    ...
    def test_subscribe_unsubscribe_flow(self):
        ...
        self.client.force_authenticate(user=self.user)
        self.client.post(f"/api/courses/{self.course.id}/subscribe/")
        self.assertTrue(Subscription.objects.filter(user=self.user, course=self.course).exists())
        self.client.delete(f"/api/courses/{self.course.id}/unsubscribe/")
    ...
    def test_pagination_applied(self):
        ...
        for _ in range(6):
            Lesson.objects.create(
                title="Extra", video_url="https://youtube.com", course=self.course, owner=self.user
            )
